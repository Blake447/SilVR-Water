Shader "SilVR Experimental/Water Surface mirrorless"
{
 Properties
 {
 ...
 SubShader
 {
     ...
     Pass
     {
         ...
         float3 intersectYPlane(float3 cam, float3 ray, float height, float3 world_x, float3 world_z)
         {
             // This might not be entirely self explanatory, but its just derived by letting the y component
             // of the ray (y_nought + bt) be equal to the height, solving for t, and plugging that into the
             // expressions for the x and z component of the ray.
         ...
         fragOut frag(v2f i)
         {
             ...
             float2 inverseRes = _MainTex_TexelSize.xy;
             ...
             float visible = step(0.5, tex2D(_AlphaMask, p_uv).x);
             visible *= step(0, dot(ray, -normal * (step(center.y, cam.y) * 2 - 1)));
