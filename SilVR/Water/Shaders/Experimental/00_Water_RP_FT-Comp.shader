Shader "SilVR Experimental/Render Plane Frame Time Compensated"
{
	Properties
	{
		...
		_TargetRate("Target Frame Rate", float) = 90
	}
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag (v2f i) : SV_Target
			{
				...
				float frameTimeComp = clamp(_TargetRate * unity_DeltaTime.y, 1.0, 2.0) * 0.5;

				// Generate the uv coordinates for sampling at the current and its 4 surround points.
				float2 uv = i.uv;
				float2 uv_u = uv + q.zy*_WaveSpeed * frameTimeComp;
				float2 uv_d = uv - q.zy*_WaveSpeed * frameTimeComp;
				float2 uv_l = uv - q.xz*_WaveSpeed * frameTimeComp;
				float2 uv_r = uv + q.xz*_WaveSpeed * frameTimeComp;


				float2 b_u = uv + q.zy*_WaveSpeed*_Bounce * frameTimeComp;
				float2 b_d = uv - q.zy*_WaveSpeed*_Bounce * frameTimeComp;
				float2 b_l = uv - q.xz*_WaveSpeed*_Bounce * frameTimeComp;
				float2 b_r = uv + q.xz*_WaveSpeed*_Bounce * frameTimeComp;
