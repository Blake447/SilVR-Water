 ...
 SubShader
 {
     ...
     Pass
     {
         ...
         float4 frag (v2f_customrendertexture i) : COLOR
         {
             ...
             float4 rp_col = tex2D(_SelfTexture2D, uv);
             float4 rp_col_u = tex2D(_SelfTexture2D, uv_u);
             float4 rp_col_d = tex2D(_SelfTexture2D, uv_d);
             float4 rp_col_l = tex2D(_SelfTexture2D, uv_l);
