Shader "SilVR Experimental/Water Surface mirrorless"
{
	Properties
	{
		...
		_ImgWidth("Image Resolution", float) = 720
		_ImgHeight("Image Resolution", float) = 720
	...
	SubShader
	{
		...
		Pass
		{
			...
			fragOut frag(v2f i)
			{
				...
				float3 dim = float3(2.5*size_x, .1*_WaveHeight, 2.5*size_y);
